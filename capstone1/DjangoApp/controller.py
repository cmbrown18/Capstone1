...
class Controller:
    ...
    def process_request(self, inp: str, user: str) -> bool:
        """
        This function will process requests generated by users. When a user generates a request, this function
            will build a well formatted string and check it against the target user's policy file. If it is in there
            then the function will end, if it is not in there then the function will add the request to the target user's
            requests file.
        """
        ...
        logger.log(rule, inp)
        valid = self.check_permissions(user, rule)

        print("This is the rule after going through the controller:")
        print(rule)
        return valid

    def check_permissions(self, user, request) -> bool:
        userfile = 'DjangoApp/Policies/' + request['target_user'] + '.txt'
        policies = open(userfile, 'r')
        line_list = list(policies.readlines())
        policies.close()
        count = 1
        found = False
        while count < len(line_list) and not found:
            parts = line_list[count].split(', ')
            requester_name = parts[0][22:len(parts[0]) - 2]
            requester_check = request['acting_user'] == requester_name
            target_doc = parts[2][26:len(parts[2]) - 2]
            target_doc_check = request['res'] == target_doc
            action = parts[1][15:len(parts[1]) - 2]
            action_check = request['action'] == action
            if requester_check and action_check and target_doc_check:
                found = True
                break
            count += 1
        return found
